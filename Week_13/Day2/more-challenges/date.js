// Exercise 1: Date #1
// In a new file called date.js, create a function that returns the amount of time left from now until January 1st. Export this function.

// In the file script.js display the result of the function created in date.js.
// (Example: the 1st January is in 10 days and 10:34:01 hours)

import { differenceInDays, differenceInHours, differenceInMinutes, format } from "date-fns";
import Holidays from "date-holidays";

export function calculateTime(){
    const today = new Date();
    const targetDate = new Date("2026-01-01T00:00:00");

    const daysLeft = differenceInDays(targetDate, today);
    const hoursLeft = differenceInHours(targetDate, today);
    const minutesLeft = differenceInMinutes(targetDate, today);

    console.log(`Time left until ${format(targetDate, "MMMM do yyyy")}:`)
    console.log(`${daysLeft} Days, ${hoursLeft} Hours, and ${minutesLeft} Minutes`);
}

// Exercise 2 : Date #2
// In a new file called date.js, create a function that accepts a birthdate as an argument (in the format of your choice), and then return the number of minutes the user lived in his life. Export this function.
// Hint: Hardcode the value of the argument, don’t ask a user for it.

// In the file script.js display the result of the function created in date.js.

// Bonus: Try to find a node module that allows to prompt the user for his birthdate.

import promptSync from "prompt-sync";

const prompt = promptSync();

export function lifeSpan(){

    const userInput = prompt("Enter your birthdate in this format YYYY-MM-DD: ");

    const today = new Date();
    const targetDate = new Date(userInput);

    return `You have lived ${differenceInMinutes(today, targetDate)} minutes in total.`;
}

// Exercise 3: Date #3
// In a new file called date.js, write a function that returns today’s date and the amount of time left from now until the next holiday,
// additionally display what holiday that is. Export this function.
// Hint: Start with hardcoding the datetime and name of the holiday.

// In the file script.js display the result of the function created in date.js.
// (Example: the next holiday is in 30 days and 12:03:45 hours)

// Bonus : Try to find a node module to get the date of the holidays instead of hardcoding it.

export function nextHoliday(countryCode){
    const country = new Holidays(countryCode);
    const today = new Date();

    // get all holidays for this year and next one
    const allHolidays = [
        ...country.getHolidays(today.getFullYear()),
        ...country.getHolidays(today.getFullYear() + 1),
    ];

    // Find the next holiday from today:
    const nextHoliday = allHolidays.find(holiday => new Date(holiday.date) > today);
    const targetDate = nextHoliday.date;

    console.log(`Today's date is: ${today}`);
    console.log(`The next ${countryCode} holiday is ${nextHoliday.name} on ${nextHoliday.date}`);
    console.log(`The amount of time left from today until the next holiday is: ${differenceInDays(targetDate, today)} days`);
}



