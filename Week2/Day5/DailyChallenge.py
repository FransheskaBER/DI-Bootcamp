...
It should take two parameters: animal_type and count (with a default value of 1). Count is the quantity of the animal that will be added to the animal
dictionary. The dictionary will look like this:
{'cow': 1, 'pig':3, 'horse': 2}
...
Step 5: Implement the get_animal_types Method
...
Step 6: Implement the get_short_info Method
...
class Farm:
    ...
    def get_info(self):
        string_list = []
        string_list.append(self.farm_name)
        for animal_type, count in self.animals.items():
            string_list.append(f"{animal_type}: {count}")
        string_list.append("     E-I-E-I-0!")
        return("\n".join(string_list))
    ...
    def get_short_info(self):
        ...
        for animal_type, count in self.animals.items():
            if count > 1:
                animal_type += "s"
            animal_names.append(f"{animal_type}")
        
        if len(animal_names) == 2:
            animal_string = " and ".join(animal_names)
        elif len(animal_names) > 2:
            animal_string = ", ".join(animal_names[:-1]) + " and " + animal_names[-1]
        print(f"{self.farm_name}'s farm has: {animal_string}")       
